...
checkRange start end s =
  let num = read s in
    num <= end && num >= start

parseField :: String -> Bool
parseField s =
  let [f1, f2] = splitBy (== ':') s in
  case f1 of
  ...
  "hgt" ->
    let (len, typ) = span isDigit f2 in
    (case typ of
      "cm" -> checkLenDigits 3 len && checkRange 150 193 len
      "in" -> checkLenDigits 2 len && checkRange 59 76 len
      _ -> False)
  "hcl" -> 
    let (hashtag, digits) = break isHexDigit f2 in
      length hashtag == 1 && head hashtag == '#' && length digits == 6
  "ecl" ->
    let eyeclrs = fromList ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"] in
    member f2 eyeclrs 
  "pid" ->
    checkLenDigits 9 f2 
  "cid" -> False
...
parseFieldLessAggressively s = 
  let [f1, _] = splitBy (== ':') s in
  member f1 $ fromList ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]

  
part_1 :: String -> String
part_1 s =
  show $ length $ filter (==7) $ fmap (length . filter id . map parseFieldLessAggressively . words) (mergeLines (/= []) unwords $ lines s)

part_2 :: String -> String
part_2 s =
  show $ length $ filter (==7) $ fmap (length . filter id . map parseField . words) (mergeLines (/= []) unwords $ lines s)
